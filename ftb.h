...
#define ftb_da_ctx(da) ((da) ? ((ftb_ctx_t*)((ftb_da_header(da))->ctx)) : FTB_RAW)
...
#define ftb_da_set_addr(da,x) do{ (ftb_da_header(da))->addr = (x);} while(0)
...
(void* ptr)
{
    ...
    ftb_ctx_t* ctx = (ftb_ctx_t*)header->ctx;
    if(ctx != FTB_RAW) {
